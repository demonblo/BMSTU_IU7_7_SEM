...
namespace fileCompression
{
    class Huffman
    {
        ...
        {
            byte[] text = Reader.ReadFull(filename);
        ...
        private static void _createTable(byte[] text, out Dictionary<byte, int> dict)
        {
        ...
        private static void _getListNodes(Dictionary<byte, int> dict, out List<TreeNode<int>> arr)
        {
            arr = new List<TreeNode<int>>();
        ...
        public static BinaryTree<int> Compress(string fileSrc, string fileDest)
        {
            ...
            byte[] text = Reader.ReadFull(fileSrc);
            
            _createTable(text, out dict);
        ...
        public static void Decompress(string fileSrc, string fileDest, BinaryTree<int> tree)
        {
            ...
            Writer.Write(res.ToArray(), fileDest);
        }
    }
}